...
# analog offset = 0 V
channel_range = 7
status["setChA"] = ps.ps4000aSetChannel(c_handle,
...
# range = PS4000A_2V = 7
# analog offset = 0 V
status["setChB"] = ps.ps4000aSetChannel(c_handle,
...
status["setDataBuffersA"] = ps.ps4000aSetDataBuffers(c_handle,
                                                     ps.PS4000A_CHANNEL['PS4000A_CHANNEL_A'],
...
# segment index = 0
# ratio mode = PS4000A_RATIO_MODE_NONE = 0
status["setDataBuffersB"] = ps.ps4000aSetDataBuffers(c_handle,
                                                     ps.PS4000A_CHANNEL['PS4000A_CHANNEL_B'],
                                                     bufferBMax.ctypes.data_as(
                                                         ctypes.POINTER(ctypes.c_int16)),
                                                     None,
                                                     sizeOfOneBuffer,
                                                     memory_segment,
...
# No downsampling:
downsampleRatio = 1
status["runStreaming"] = ps.ps4000aRunStreaming(c_handle,
...
status["maximumValue"] = ps.ps4000aMaximumValue(c_handle, ctypes.byref(maxADC))
...
# Create time data
time = np.linspace(0, (totalSamples - 1) *
                   actualSampleIntervalNs, totalSamples)
...
# Stop the scope
# handle = c_handle
status["stop"] = ps.ps4000aStop(c_handle)
assert_pico_ok(status["stop"])

# Disconnect the scope
# handle = c_handle
status["close"] = ps.ps4000aCloseUnit(c_handle)
